...
with open('plat.tet1_gk.xml', 'r') as file:
    content = file.read()

content=re.sub(r'(<text n=")(\w+\.?)(">)', r'\$ \2\n', content)
...
content=re.sub(r'<bibl n?=?"?\w*\.?\s?\w*\.?\s?\d*\.?\d*"?>\w*\.?\w*\s?\w*\.?\s?\d*\.?\d*<?/?b?i?b?l?>?', r'', content)
...
works=re.split('\$', content, maxsplit=5)
...
final_file1 = open('Plato_Eutiph.xml', 'w')
final_file1.write(work1)
final_file1.close()

final_file2 = open('Plato_Apol.xml', 'w')
final_file2.write(work2)
final_file2.close()

final_file3 = open('Plato_Crit.xml', 'w')
final_file3.write(work3)
final_file3.close()

final_file4 = open('Plato_Phaed.xml', 'w')
